#!/usr/bin/env python3
"""
Generate index.html file with links to all PDF files in the backnumber directory.
"""
...
def parse_pdf_filename(filename):
    """
    Parse PDF filename to extract date and slide count information.

    Expected format: mobile_slides_YYYYMMDD_HHMMSS_XXslides.pdf

    Args:
        filename: PDF filename to parse

    Returns:
        dict with 'date', 'time', 'slides' keys, or None if parsing fails
    """
    pattern = r'mobile_slides_(\d{8})_(\d{6})_(\d+)slides\.pdf'
    match = re.match(pattern, filename)

    if match:
        date_str, time_str, slides = match.groups()
        try:
...
def get_pdf_files(directory):
    ...
    Returns:
        List of tuples (filename, file_info_dict)
    """
    pdf_files = []

    for file in Path(directory).glob('*.pdf'):
        info = parse_pdf_filename(file.name)
        pdf_files.append((file.name, info))

    # Sort by date (newest first)
    pdf_files.sort(key=lambda x: x[1]['datetime'] if x[1] else datetime.min, reverse=True)
...
<body>
    <div class="top-bar">HANA NEWS AGENCY - OFFICIAL PRESS RELEASES</div>

    <header>
        <div class="header-content">
            <div class="news-title">
                <h1>はな通信社</h1>
                <div class="title-en">Hana News Agency</div>
            </div>
            <div class="tagline">社主が厳選したニュースをお届け | 最新情報をお届けします</div>
        </div>
    </header>

    <div class="container">
        <div class="breadcrumb">
            ホーム / <span class="breadcrumb-current">配信情報バックナンバー</span>
        </div>
        ...
        <div class="main-content">
            ...
            <div class="stats">
                ...
                <div class="stats-text">公開中の配信記事: <strong>''' + str(len(pdf_files)) + '''</strong> 件</div>
            </div>
    ...
    for filename, info in pdf_files:
        if info:
            html_content += f'''                <li class="pdf-item">
                    <a href="backnumber/{filename}" class="pdf-link" target="_blank">
                        <div class="pdf-info">
                            <div class="pdf-title">{info['date']} {info['time']}に作成したニュース</div>
                            <div class="pdf-meta">
            ...
            html_content += f'''                <li class="pdf-item">
                    <a href="backnumber/{filename}" class="pdf-link" target="_blank">
                        <div class="pdf-info">
                            ...
                            <div class="pdf-meta">
                                <span class="meta-item">PDFファイル</span>
                            </div>
                        </div>
                    </a>
                </li>
'''
    ...
    <footer>
        <div class="footer-content">
            <div class="footer-logo">Hana News Agency</div>
            <p class="footer-text">&copy; 2025 Hana News Agency. All rights reserved.</p>
            <p class="footer-text">社主が厳選したニュース配信をお届けします</p>
            <p class="generated-time">最終更新: {current_time}</p>
        </div>
    </footer>
</body>
</html>
